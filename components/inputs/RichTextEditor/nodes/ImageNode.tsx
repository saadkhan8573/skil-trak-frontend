...
function ImageComponent({ src, altText, width, height, maxWidth, nodeKey, alignment }: any) {
    ...
    const onAlignmentChange = useCallback((newAlignment: ImageAlignment) => {
        editor.update(() => {
    ...
    const onResize = useCallback((newWidth: string) => {
        editor.update(() => {
            const node = $getNodeByKey(nodeKey);
            if ($isImageNode(node)) {
    ...
    const onUploadToServer = useCallback(async () => {
        ...
        if (!fileToUpload) {
            console.warn('ImageNode: 🔄 Trying Phase 2 (Transient CORS Test)...');
            try {
                ...
                await new Promise((resolve, reject) => {
                    ...
                    testImg.onerror = () => reject(new Error('CORS blocked or Load failed'));
                    // Timeout after 5s
                ...
                console.warn('ImageNode: ⚠️ Phase 2 (Direct Capture) failed. Source does not allow mirroring.', canvasErr);
            }
        }
        ...
        function blobToFile(blob: Blob, url: string): File {
            ...
            const fileNameFromUrl = (url.split('/').pop() || 'image').split('?')[0].split('#')[0];
    ...
    const getAlignmentStyles = () => {
        switch (alignment) {
            case 'left':
                return { float: 'left' as const, marginRight: '20px', marginBottom: '10px' };
            case 'right':
                return { float: 'right' as const, marginLeft: '20px', marginBottom: '10px' };
            case 'center':
                return { display: 'block', marginLeft: 'auto', marginRight: 'auto', marginBottom: '10px' };
            case 'full':
    ...
    return (
        <div
            ...
            style={{
                position: 'relative',
                display: (alignment === 'center' || alignment === 'full' || (typeof width === 'string' && width.endsWith('%'))) ? 'block' : 'inline-block',
                clear: 'both',
                width: (typeof width === 'string' && width.endsWith('%')) ? width : 'auto',
                ...getAlignmentStyles()
            }}
        >
            <div style={{
...
export class ImageNode extends DecoratorNode<React.ReactNode> {
    ...
    static importJSON(serializedNode: SerializedImageNode): ImageNode {
        const { altText, height, width, maxWidth, src, showCaption, alignment } =
            serializedNode;
