...
const SECTOR_CONFIGS = [
    { name: 'Community Services', icon: '🏠', color: '#044866', bgColor: 'rgba(4, 72, 102, 0.05)' },
    { name: 'Disability', icon: '♿', color: '#0D5468', bgColor: 'rgba(13, 84, 104, 0.05)' },
    { name: 'Mental Health', icon: '🧠', color: '#044866', bgColor: 'rgba(4, 72, 102, 0.05)' },
    { name: 'Health Care', icon: '⚕️', color: '#0D5468', bgColor: 'rgba(13, 84, 104, 0.05)' },
    { name: 'Individual Support', icon: '🤝', color: '#044866', bgColor: 'rgba(4, 72, 102, 0.05)' },
]
...
export function SectorsCard({ data, onChange, errors, handleAddSector, uniqueSectors }: any) {
    const [expandedSector, setExpandedSector] = useState<string | null>(
        uniqueSectors?.length > 0 ? uniqueSectors[0]?.id : null
    )
    const [selectedIndustryCheck, setSelectedIndustryCheck] = useState<string | null>(null)
    ...
    const { data: questions, isLoading: questionsLoading, isError: questionsError } =
        AdminApi.SectorClusters.useSectorClusterQuestions(Number(expandedSector), {
            skip: !expandedSector,
        })

    const { data: coursesByLevel, isLoading: coursesLoading, isError: coursesError } =
    ...
    const toggleSectorExpansion = (sectorId: any) => {
        setExpandedSector((prev) => (String(prev) === String(sectorId) ? null : sectorId))
    }

    const removeSector = (sectorId: any) => {
        onChange({
            ...data,
            sectors: data.sectors.filter((s: any) => String(s.id) !== String(sectorId)),
        })
        setExpandedSector((prev) => (String(prev) === String(sectorId) ? null : prev))
    }
    ...
    const updateSector = (sectorId: any, updates: any) => {
        ...
        const isCluster = clusterSectors.some((s: any) => String(s.id) === String(sectorId))
            ...
            onChange({
                ...
                sectors: [
                    ...(uniqueSectors?.map((s: any) => {
                        const existing =
                            data.sectors?.find(
                                (ds: any) => !ds.isClusterSector && String(ds.id) === String(s.id)
                            ) || {}
    ...
    const updateIndustryCheck = (sectorId: string, checkId: any, updates: Record<string, any>) => {
        const sectorState = data.sectors.find((s: any) => String(s.id) === String(sectorId))
        const existing = sectorState?.industryChecks || []
        const alreadyTracked = existing.some((c: any) => String(c.id) === String(checkId))
    ...
    const toggleCustomCheck = (sectorId: string, enabled: boolean) => {
        ...
        const current = (sectorState as any)?.customCheck || {
            name: '', capacity: '', link: '', description: '', required: false, enabled: false,
        }
        updateSector(sectorId, { customCheck: { ...current, enabled } } as any)
    }

    const updateCustomCheckField = (sectorId: string, field: string, value: any) => {
        const sectorState = data.sectors.find((s: any) => s.id === sectorId)
        const current = (sectorState as any)?.customCheck || {
            name: '', capacity: '', link: '', description: '', required: false, enabled: true,
        }
        updateSector(sectorId, { customCheck: { ...current, [field]: value } } as any)
    }

    const toggleQuestion = (sectorId: string, questionId: number, checked: boolean) => {
        const sectorState = data.sectors.find((s: any) => s.id === sectorId)
        updateSector(sectorId, {
            questionChecks: { ...(sectorState?.questionChecks || {}), [questionId]: checked },
        })
    }

    const toggleTask = (sectorId: string, courseId: number, taskId: number) => {
        const current: Record<string, Record<number, number[]>> = data.selectedTaskIds || {}
    ...
    const areAllQuestionsChecked = (sectorId: any) => {
        ...
        return questions.every((q: any) => checks[q.id])
    }

    const isSectorReadyToConfirm = (sectorId: any) => {
        const s = data.sectors.find((ds: any) => String(ds.id) === String(sectorId))
        if (!s) return false
        return !!(s.supervisorName?.trim() && s.supervisorLevel && s.qualificationTitle?.trim() && s.capacity >= 1)
    }
    ...
    const handleConfirmSector = (sectorId: any) => {
        ...
        setConfirmedSectorsData((prev) => {
            ...
            if (exists) {
                return prev.map((s) =>
                    String(s.id) === String(sectorId) ? { ...sectorState, confirmed: true } : s
                )
            }
        ...
        const currentIndex = allSectorsNow.findIndex((s: any) => String(s.id) === String(sectorId))
    ...
    const handleAddClusterSector = (clusterSector: any) => {
        if (data.sectors?.some((s: any) => String(s.id) === String(clusterSector.id))) return
        onChange({
            ...data,
            sectors: [
                ...(data.sectors || []),
        ...
        >
            <Card noPadding className="border-0 shadow-xl bg-linear-to-br from-white to-gray-50/30 hover-lift">
                <div
                    ...
                    style={{ background: `linear-gradient(135deg, rgba(4, 72, 102, 0.08) 0%, rgba(13, 84, 104, 0.05) 100%)` }}
                >
                    <div className="flex items-center gap-3">
                        <div
                            className="size-10 rounded-xl flex items-center justify-center shadow-lg"
                            style={{ background: `linear-gradient(135deg, #044866 0%, #0D5468 100%)` }}
                        >
                            <Building2 className="size-6 text-white" />
                        </div>
                        <div>
                            <h3 className="gradient-text text-lg">Industry Sectors Configuration</h3>
                            <p className="text-xs text-muted-foreground">
                                🏢 Set up sectors where you can provide quality professional placements
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-8 p-8">
                    <div className="space-y-6">
                        <div className="flex items-center justify-between">
                            <div>
                                <Label className="text-base font-semibold">Your Active Sectors</Label>
                        ...
                        <AnimatePresence>
                            {allSectors.map((sector: any, index: number) => {
                                const sectorConfig = getSectorConfig(sector.name)
                                const isExpanded = String(expandedSector) === String(sector.id)
                                const sectorState = data.sectors.find(
                                    (s: any) => String(s.id) === String(sector.id)
                                )
                                ...
                                return (
                                    <SectorItem
                                        ...
                                        errors={errors || {}}
                                        allQuestionsChecked={areAllQuestionsChecked(sector.id)}
                                        isReadyToConfirm={isSectorReadyToConfirm(sector.id)}
                                        onToggleExpand={() => toggleSectorExpansion(sector.id)}
                                        onRemove={() => {
                                            if (sector.isClusterSector) {
                                                removeSector(sector.id)
                                            } else {
                                                // TODO: Remove via delete API when available
                                                console.log('Remove API sector via delete endpoint:', sector.id, sector.name)
                                            }
                                        }}
                                        onUpdateSector={(updates) => updateSector(sector.id, updates)}
                                        onUpdateIndustryCheck={(checkId, updates) =>
                                            updateIndustryCheck(sector.id, checkId, updates)
                                        }
                                        onToggleCustomCheck={(enabled) => toggleCustomCheck(sector.id, enabled)}
                                        onUpdateCustomCheckField={(field, value) =>
                                            updateCustomCheckField(sector.id, field, value)
                                        }
                                        onToggleQuestion={(questionId, checked) =>
                                            toggleQuestion(sector.id, questionId, checked)
                                        }
                                        onToggleTask={(courseId, taskId) =>
                                            toggleTask(sector.id, courseId, taskId)
                                        }
                                        onQualificationLevelChange={setQualificationLevel}
                                        onConfirmSector={() => handleConfirmSector(sector.id)}
                                    />
                                )
                            })}
                        </AnimatePresence>
                        ...
                        {allSectors.length === 0 && (
                            <div className="text-center py-12 text-muted-foreground">
                                ...
                                <p>No sectors configured yet. Add your first sector above to get started.</p>
                            </div>
                        )}
                    </div>
                </div>
