...
export const StartMatchingAutoWP = ({ onCancel }: { onCancel: () => void }) => {
    ...
    const [accumulatedResults, setAccumulatedResults] = useState<RtoApprovalWorkplaceRequest[]>([])
    ...
    const resultData = isProcessing || hasFinished || accumulatedResults.length > 0 ? accumulatedResults : applyResult?.data
    ...
    const onSubmit = useCallback(async () => {
        ...
        for (let i = 0; i < total; i++) {
            ...
            try {
                ...
                const errorMsg = error?.data?.message || error?.message || "Technical Error"
    ...
    useEffect(() => {
        if (!wpAutoMatchingList.isFetching && shouldAutoStart && wpAutoMatchingList.isSuccess) {
            setShouldAutoStart(false)
            onSubmit()
        }
    }, [wpAutoMatchingList.isFetching, shouldAutoStart, wpAutoMatchingList.isSuccess, onSubmit])
    ...
    return (
        <GlobalModal className="overflow-hidden! max-w-3xl!">
            ...
            <MdCancel
                ...
                className={`z-30 transition-all duration-500 text-gray-200 hover:text-gray-100 text-3xl cursor-pointer hover:rotate-90 absolute top-2 right-2 ${isProcessing ? 'opacity-50 cursor-not-allowed' : ''}`}
            />
            <div
                className={
                    ...
                    <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 flex-wrap">
                                ...
                                {(isProcessing || (wpAutoMatchingList.isFetching && shouldAutoStart)) && (
                                    <Badge
                                        variant="warning"
                ...
                <div className="bg-orange-50 border-b border-orange-100 px-6 py-3 flex items-center gap-3 animate-in fade-in slide-in-from-top-2">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-orange-700">
                            Please do not close this modal or refresh the page. We are processing requests one by one to ensure system stability.
                        </p>
                    </div>
                    ...
                    <div className="space-y-4">
                        {!isProcessing && !isFinished && (
                            <div className="bg-gray-50 p-3 rounded-md border border-gray-100 italic">
                                <Typography className="text-gray-600 text-sm">
                                    <span className="font-semibold text-gray-700">Note:</span> Automation will run sequentially to maintain performance.
                                </Typography>
                            </div>
                        )}
                        ...
                        {isProcessing && (
                            <div className="w-full bg-gray-100 rounded-full h-2 mb-4">
                                <div
                                    ...
                                    style={{ width: `${((currentIndex + 1) / wpAutoMatchingList.data.length) * 100}%` }}
                                />
                            </div>
                        )}
                        ...
                        <div>
                            ...
                            <div className="space-y-3">
                                {wpAutoMatchingList?.data?.map((workplace, index) => {
                                    const result = accumulatedResults?.find(
                                    ...
                                    const isWaiting = index > currentIndex && currentIndex !== -1
                                    const isFailed = failedIndices.includes(index)
                                    const isChecked = index < currentIndex || (isFinished && !result && !isFailed)

                                    return (
                                        <div
                                            key={workplace?.id}
                                            {...(isCurrent ? { ref: activeItemRef } : {})}
                                            className="w-full"
                                        >
                                            <Card
                                                    ...
                                                    <div className="flex items-start justify-between gap-4">
                                                        <div className="flex-1 min-w-0">
                                                            <div className="flex items-center gap-2 mb-2">
                                                                ...
                                                                {isFailed && (
                                                                    <Badge
                                                                        text={errorMessages[index] || "Technical Error"}
                                                                        variant="error"
                                                                        Icon={AlertCircle}
                                                                    />
                                                                )}
                                                                    ...
                                                                    <div className="flex items-start gap-2">
                                                                        ...
                                                                        <div className="flex-1">
                                                                            ...
                                                                            <p className="text-xs text-red-700 mt-1">
                                                                                There was an error processing this request. This could be due to network or server issues.
                                                                            </p>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                                        ...
                                                                        <div className="flex items-start gap-2">
                                                                            ...
                                                                            <div className="flex-1">
                                                                                ...
                                                                                <p className="text-xs text-orange-700 mt-1">
                                                                                    System couldn't find an automatic match that satisfies all constraints.
                                                                                </p>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                    ...
                                    <div className="grid grid-cols-3 gap-4">
                                        ...
                                        <div className="p-4 rounded-xl bg-orange-50 border border-orange-200 shadow-sm">
                                            <p className="text-3xl font-bold text-orange-600 mb-1">
                                                {(wpAutoMatchingList?.data?.length || 0) - (resultMatched?.length || 0) - (failedIndices?.length || 0)}
                                            </p>
                                            <p className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">
                                                Manual
                                            </p>
                                    ...
                                    <div className="mt-6 space-y-4">
                                        <div className="p-3 bg-white/50 border border-border/40 rounded-lg">
                                            <p className="text-xs text-center text-muted-foreground">
                                                Automation process completed. You can now safely close this window or review individual matches.
                                            </p>
                                        </div>
                                        <Button
                                            onClick={handleReset}
