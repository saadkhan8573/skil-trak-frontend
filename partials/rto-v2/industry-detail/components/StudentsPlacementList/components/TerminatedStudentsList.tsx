...
export function TerminatedStudentsList() {
    ...
    const workplaceTerminatedStudents = RtoV2Api.Industries.getIndustryTerminatedStudents(
        {
    ...
    return (
        <div className="space-y-2">
            ...
            {workplaceTerminatedStudents?.isLoading || workplaceTerminatedStudents?.isFetching ? (
                <StudentsTabSkeleton />
                ...
                <>
                    <div className="flex items-center justify-between">
                        <PageSize
                            ...
                            records={workplaceTerminatedStudents?.data?.data?.length}
                        />
                        <Pagination
                            setPage={setPage}
                            pagination={workplaceTerminatedStudents.data?.pagination}
                            updateUrl={false}
                        />
                    </div>
                    {workplaceTerminatedStudents?.data?.data?.map((workplace) => (
                        <TerminatedStudentCard key={workplace.id} workplace={workplace} />
                    ))}
                </>
            ) : workplaceTerminatedStudents?.isSuccess ? (
                <EmptyData
