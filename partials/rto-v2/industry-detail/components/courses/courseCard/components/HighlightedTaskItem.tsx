import { Badge, Permissions } from '@components'
import { Tooltip, TooltipContent, TooltipTrigger } from '@components/ui'
import { useAppSelector } from '@redux'
import { ConfirmationSource, PermissionType } from '@types'
    ...
    return (
        <>
            <motion.div
                ...
                className={`p-2.5 rounded-lg border transition-all duration-200 group border-gray-400/80 ${
                    isSelected
            ...
            >
                ...
                <div className="flex items-start gap-2.5">
                    {(!confirmationDetail || !confirmationDetail.isConfirmed) &&
                        ...
                        !isDeleted && (
                            <div className="mt-0.5 shrink-0">
                                <div
                                    className={`w-4 h-4 rounded border flex items-center justify-center transition-colors ${isSelected ? 'bg-[#044866] border-[#044866]' : 'border-gray-300'}`}
                                >
                                    {isSelected && (
                    ...
                    <div className="flex-1 min-w-0">
                        <p
                            className={`text-[12px] leading-tight font-semibold line-clamp-2 ${isSelected ? 'text-[#044866]' : 'text-gray-800'}`}
                        >
                            {task.statement}
                        </p>
                    </div>
                    ...
                    <div className="flex items-center gap-2 pt-1 border-t border-gray-100/50">
                        ...
                        <div className="flex items-center flex-wrap gap-1.5 text-[10px] text-gray-500 min-w-0">
                            {confirmationDetail.isConfirmed && (
                                <>
                                    <span>via</span>
                                    {confirmationDetail.confirmationSource ===
                                    ...
                                    <span className="font-bold text-[#044866]">
                                        {confirmationDetail.confirmedBy?.name?.split(
                                            ' '
                                        )[0] || 'Admin'}
                                    </span>
                                </>
