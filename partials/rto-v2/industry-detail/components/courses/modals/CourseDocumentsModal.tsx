...
import { Badge, Button, LoadingAnimation, NoData, ShowErrorNotifications, Typography } from '@components'
...
export function CourseDocumentsModal({
    ...
    const sectors = IndustryApi.Courses.useGetIndustrySectorsQuery(
        ...
        { refetchOnMountOrArgChange: true, skip: !industryUserId }
    )

    const esignList = CommonApi.ESign.industryAllEsigns({ id: industryId, sectorId }, {
        skip: !industryId,
    ...
    const getStatusIcon = (document: any) => {
        ...
        if (document?.status === 'signed' && !document?.initiatedBy) {
            return <FileCheck2 className="h-4 w-4" style={{ color: '#0D5468' }} />
        }
        if (!document) {
    ...
    // Exact same mapping as IndustryEsignList
    ...
    console.log({ sectorsAndTemplates })
    ...
    const allTemplates = sectorsAndTemplates?.flatMap((s: any) => s.templates || []) || []
    const totalTemplates = allTemplates.length
    const pendingActions = allTemplates.filter((t: any) => !t?.documents?.[0]).length
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            ...
            <ConfirmBulkInitiateModal
                ...
                templateIds={sectorsAndTemplates?.[0]?.templates?.map((t: any) => Number(t.id)) || []
                }
            />
            <DialogContent className="sm:max-w-[900px] h-[85vh] p-0 flex flex-col bg-white rounded-2xl border-none shadow-2xl overflow-hidden">
                ...
                <div className="bg-gradient-to-r from-[#044866] to-[#0D5468] p-6 text-white shrink-0">
                    <DialogHeader>
                        ...
                        <DialogDescription className="text-teal-100/80">
                            Manage electronic signatures and view course-related documents
                        </DialogDescription>
                    </DialogHeader>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center justify-between border-b border-gray-100 pb-2">
                            <div className="flex items-center gap-3">
                                <h3 className="text-lg font-semibold text-[#1A2332]">Initiate Industry Esign</h3>
                                <span className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded-full">{sectorsAndTemplates?.flatMap((s: any) => s.templates || [])?.length || 0} Templates</span>
                            </div>
                            {sectorsAndTemplates && sectorsAndTemplates?.length > 0 && (
                                <Button
                                    variant="primaryNew"
                        ...
                        {sectors.isLoading || esignList.isLoading ? (
                            <LoadingAnimation />
                        ) : sectorsAndTemplates.some((s: any) => s.templates?.length > 0) ? (
                            <div className="space-y-6">
                                {sectorsAndTemplates.map((sector: any) => (
                                    sector.templates?.length > 0 && (
                                        <div key={sector.id} className="space-y-4">
                                            {/* We only show the sector name if there are multiple sectors with templates, 
                                                but since this modal is for one sector, we might just show templates.
                                                However, to keep "exact same logics", we support the structure. */}
                                            {sectorsAndTemplates.filter((s: any) => s.templates?.length > 0).length > 1 && (
                                                <h4 className="text-sm font-medium text-gray-500">{sector.name}</h4>
                                            )}
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                {sector.templates.map((template: any) => {
                                                    const doc = template?.documents?.[0]
                                                    const isSigned = doc?.status === 'signed' && !doc?.initiatedBy
                                                    const isSent = doc?.initiatedBy

                                                    return sector?.id === template?.sector?.id && (
                                                        <div
                                                            key={template.id}
                                                            className={`relative group rounded-xl transition-all duration-300 hover:shadow-2xl hover:-translate-y-1 bg-white border-2 overflow-hidden cursor-pointer ${selectedTemplate?.id === template.id
                                                        ...
                                                        >
                                                            {/* Status Glow Overlay */}
                                                            <div className={`absolute inset-0 opacity-0 group-hover:opacity-[0.03] transition-opacity duration-300 pointer-events-none ${isSigned ? 'bg-emerald-500' : isSent ? 'bg-blue-500' : 'bg-amber-500'
                                                                }`}></div>

                                                            {/* Colorful Background Blob */}
                                                            <div className={`absolute top-0 right-0 w-20 h-20 rounded-full blur-2xl -mr-10 -mt-10 transition-all opacity-40 ${isSigned ? 'bg-emerald-200' : isSent ? 'bg-blue-200' : 'bg-amber-200'
                                                                }`}></div>

                                                            {/* Card Content */}
                                                            <div
                                                                className={`relative p-3 h-full flex flex-col transition-all duration-300 ${isSent || isSigned ? 'border-2 border-primaryNew bg-primaryNew-light' : 'border-2 border-primary bg-[#F7910F08]'
                                                                    }`}
                                                                style={{
                                                                    ...(selectedTemplate?.id === template.id && ({
                                                                        ringColor: isSent ? '#044866' : isSigned ? '#0D5468' : '#F7A619',
                                                                    } as any)),
                                                                }}
                                                            >
                                                                ...
                                                                <div className="flex items-start gap-3 mb-3">
                                                                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-white shadow-md transition-transform duration-300 shrink-0 ${isSigned ? 'bg-emerald-600 shadow-emerald-200' :
                                                                        isSent ? 'bg-blue-600 shadow-blue-200' :
                                                                            'bg-primaryNew shadow-primaryNew'
                                                                        } group-hover:scale-110`}>
                                                                        {getStatusIcon(doc)}
                                                                    </div>
                                                                    <div className="pr-14 min-w-0">
                                                                        <h4 className="font-bold text-slate-800 text-sm leading-tight mb-1 line-clamp-2" title={template.name}>
                                                                            {template.name}
                                                                        </h4>
                                                                        {doc?.initiatedBy && (
                                                                    ...
                                                                    <div className="grid grid-cols-2 gap-2">
                                                                        ...
                                                                        {!doc ? (
                                                                            <Button
                                                                                ...
                                                                                onClick={(e) => {
                                                                                    e.stopPropagation()
                                                                                    onInitiateSign(Number(template.id))
                                                                                }}
                            ...
                            <div className="bg-gray-50 rounded-xl p-8 text-center border border-dashed border-gray-200">
                                ...
                                <p className="text-gray-500 text-sm">No esign templates available for this sector.</p>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center justify-between border-b border-gray-100 pb-2">
                            <h3 className="text-lg font-semibold text-[#1A2332]">Documents List</h3>
                            <span className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded-full">{getPendingEsign?.data?.data?.length || 0} Files</span>
                        </div>
                        ...
                        {getPendingEsign?.isLoading || getPendingEsign?.isFetching ? (
                        ...
                        ) : getPendingEsign?.data?.data && getPendingEsign?.data?.data?.length > 0 ? (
                                        ...
                                        >
                                            ...
                                            <div className="relative flex flex-col items-center text-center gap-3">
                                                ...
                                                <div className="w-full text-center">
                                                    <p className="text-[11px] font-bold text-slate-700 line-clamp-2 leading-snug group-hover:text-blue-700 transition-colors" title={document?.file}>
                                                        {document?.file?.split('/')?.pop()?.split('\\')?.pop()?.replaceAll('{"', '')?.replaceAll('"}', '') || 'Document'}
                                                    </p>
                            ...
                            <div className="bg-gray-50 rounded-xl p-8 text-center border border-dashed border-gray-200">
                                ...
                                <p className="text-gray-500 text-sm">No documents found for this sector.</p>
                            </div>
                        )}
                    </div>
                </div>
                <div className="p-4 border-t bg-gray-50 flex items-center justify-between">
                    <div className="text-sm text-gray-600">
                        {esignList?.data?.pagination?.totalResult || 0} total templates • {pendingActions} pending actions
                    </div>
                    <div className="flex gap-3">
                        <Button variant="secondary" onClick={() => onOpenChange(false)} className="bg-white border-slate-200">
                            Cancel
                        </Button>
                        <Button
                            variant="primaryNew"
                            ...
                            loading={initiateResult.isLoading}
                            disabled={!selectedTemplate || initiateResult.isLoading}
