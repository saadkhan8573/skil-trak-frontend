        ...
        <div className="flex items-start gap-4 flex-1 min-w-0">
            ...
            <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5 mb-1.5 text-wrap">
                    <Typography variant="title" color={'text-[#1A2332]'} bold className="text-xl">
                        ...
                        {details.map((detail, idx) => (
                            detail.value ? (
                                <div key={idx} className="flex items-center gap-1.5 text-[11px] text-gray-700">
                                    ...
                                    <span className="font-semibold text-gray-500 whitespace-nowrap">{detail.label}:</span>
                                    {detail.isLink ? (
                                        <Link
                                        ...
                                        <span className="truncate max-w-[200px]">{detail.value}</span>
                                    )}
                                </div>
                            ) : null
                        ))}
