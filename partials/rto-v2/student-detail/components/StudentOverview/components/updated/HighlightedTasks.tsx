import { Badge, Button, NoData, SkeletonLoader } from '@components'
import { RtoV2Api } from '@queries'
import { IWorkplaceIndustries } from '@redux/queryTypes'
import { AnimatePresence, motion } from 'framer-motion'
import { CheckCheck, CheckCircle2, ChevronRight, Zap } from 'lucide-react'

export function HighlightedTasksSkeleton() {
    return (
        <div className="bg-white/80 backdrop-blur-sm rounded-xl border border-slate-200/60 shadow-lg shadow-slate-200/50 overflow-hidden">
            ...
            <div className="p-5 space-y-4">
                {[1, 2, 3].map((i) => (
                    <div
                        key={i}
                        className="flex items-start gap-3 p-3.5 rounded-xl border border-slate-100 bg-slate-50/50"
                    >
                        <SkeletonLoader
                            width="w-6"
                            height="h-6"
...
export function HighlightedTasks({
    ...
    const { data: highlightedData, isLoading } =
        RtoV2Api.PlacementRequests.useIndustryPlacementHighlightedTasks(
            workplace?.id,
            { skip: !workplace?.id }
        )

    const [confirmTasks, { isLoading: isConfirming }] =
    ...
    const isConfirmed = isString
        ? Boolean(
              highlightedData?.courseConfigurationDetail?.highlightedTask
                  ?.isConfirmed
          )
    ...
    if (isLoading) {
        return <HighlightedTasksSkeleton />
    }
    ...
    return (
        <div className="bg-white/80 backdrop-blur-sm rounded-xl border border-slate-200/60 shadow-lg shadow-slate-200/50 overflow-hidden hover:shadow-xl transition-all">
            ...
            <div className="bg-linear-to-r from-[#F7A619] to-amber-500 px-5 py-3 relative overflow-hidden">
                ...
                <div className="relative flex items-center gap-2.5">
                    ...
                    <div>
                        ...
                        <p className="text-white/80 text-xs text-nowrap">
            ...
            {/* Tasks List */}
            <div className="p-5 space-y-3 max-h-90 overflow-auto">
                {tasks.length > 0 ? (
                    <AnimatePresence>
                        {tasks.map((task: any, index: number) => {
                            const taskText = isString
                                ? task
                                : task?.courseHighlightedTask?.statement ||
                                  '---'
                            return (
                                <motion.div
                                    key={task.id || index}
