...
export const StudentWorkplaceComplianceChecks = () => {
    ...
    if (isLoading) return <div className="p-4 bg-white rounded-lg border border-emerald-200 shadow-sm"><LoadingAnimation /></div>

    if (!wpId || complianceItems.length === 0) return (
        <div className="relative overflow-hidden rounded-lg bg-linear-to-br from-emerald-50 to-white border border-emerald-200 shadow-sm p-4 text-center">
            ...
            <p className="text-xs text-slate-500 font-medium">No compliance checks found for this workplace</p>
        </div>
    )
    ...
    return (
        <div className="relative overflow-hidden rounded-lg bg-linear-to-br from-emerald-50 to-white border border-emerald-200 shadow-sm p-2 hover:shadow-md transition-all duration-300">
            <div className="flex items-center gap-2 mb-2">
                <div className={`w-10 h-10 rounded-xl bg-linear-to-br flex items-center justify-center shadow-lg transition-all duration-500 ${isAllApproved ? 'from-emerald-500 to-emerald-600' : 'from-amber-400 to-amber-500'}`}>
                    <Shield className="w-5 h-5 text-white" />
                </div>
                <div>
                    <h3 className="text-sm font-bold text-slate-900">
                        {isAllApproved ? 'Compliance Verified' : 'Compliance Pending'}
                    </h3>
                ...
                {isAllApproved && (
                    <div className="ml-auto">
                        <Badge text="Completed" className="bg-emerald-100 text-emerald-700 border-emerald-200 text-[10px]" />
                    </div>
                )}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                {complianceItems.map((item, index) => (
                    ...
                    >
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center shadow-sm shrink-0 transition-all duration-300 group-hover/doc:scale-110 ${item.isUploaded ? 'bg-emerald-100' : 'bg-amber-100'}`}>
                            {item.isUploaded ? (
                        ...
                        <div className="min-w-0 flex-1">
                            ...
                            <div className="flex items-center gap-1.5 mt-0.5">
                                <span className={`text-[9px] font-semibold px-1.5 py-0.5 rounded-full ${item.fileStatus === 'approved' ? 'bg-emerald-50 text-emerald-600' :
                                        item.isUploaded ? 'bg-blue-50 text-blue-600' : 'bg-amber-50 text-amber-600'
                                    }`}>
                                    {item.fileStatus === 'approved' ? 'Approved' : item.isUploaded ? 'Uploaded' : 'Pending'}
                                </span>
                                <span className="text-[9px] text-slate-300">•</span>
                                <p className="text-[9px] font-medium text-slate-500">
                                    {item.isUploaded ? `${item.filesCount} docs` : 'No file'}
                                </p>
                            </div>
                        </div>
                    </motion.div>
