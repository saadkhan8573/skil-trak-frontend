    ...
    useEffect(() => {
        if (isSuccess) {
            alert.success({
                ...
                description: `Student "${student?.user?.name}" has been ${mode}ed.`,
            })
            onCancel()
        }
        if (isError) {
        ...
        <Dialog open={true} onOpenChange={onCancel}>
            <DialogContent className="sm:max-w-md bg-white">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Are you sure you want to {mode.toLowerCase()} <strong>"{student?.user?.name}"</strong>?
                    </DialogDescription>
                </DialogHeader>
                <DialogFooter className="sm:justify-end gap-2">
                    <Button variant="secondary" outline onClick={onCancel} disabled={isLoading}>
                        Cancel
                    </Button>
                    <Button
                        onClick={onConfirmClicked}
                        disabled={isLoading}
                        className={isAssign ? 'bg-primaryNew hover:bg-primaryNew/90' : 'bg-red-600 hover:bg-red-700 text-white'}
