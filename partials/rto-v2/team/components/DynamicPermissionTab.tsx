...
interface DynamicPermissionTabProps {
    userId?: number
}

export const DynamicPermissionTab: FC<DynamicPermissionTabProps> = ({
    userId,
}) => {
    const { notification } = useNotification()
    const { data: permissionsData } =
        AdminApi.Permissions.useMyPermissions(undefined)
    ...
    const groupedPermissions = useMemo(() => {
        ...
        permissionsData.forEach((item: IAssignedPermission) => {
            const permission = item.permission as IPermission
        ...
        <div className="space-y-4 animate-fade-in">
            ...
            <Card className="flex flex-col md:flex-row items-center gap-4 bg-white/50 backdrop-blur-md p-1.5 rounded-xl border border-white/20 shadow-xl mb-6">
                <div className="relative flex-1 w-full group rounded">
                    ...
                    <TextInput
                        name="search"
                        placeholder="Search coordinator capabilities..."
                        value={searchQuery}
